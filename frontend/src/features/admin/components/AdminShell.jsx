...
function AdminMovies({ movies, moviesLoading }) {
  return (
    <AdminCard title="Movie catalog">
      ...
      {moviesLoading ? <p className="admin-muted">Loading movies...</p> : <MovieTable movies={movies} />}
    </AdminCard>
  );
}
...
function MovieTable({ movies }) {
  const displayMovies = movies.length ? movies : [{ title: "Arrival", genres: "Sci-Fi", runtime: "116 min", status: "showing" }];
